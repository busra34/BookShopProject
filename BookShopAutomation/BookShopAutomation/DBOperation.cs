...
namespace Project
{
    class DBOperation
    {
        ...
        public ArrayList GetUser(string userName, string password) //Kullanıcı bilgilerini getirme
        {
            try
            {
                connection.Open();
        ...
        public ArrayList GetRegUser(string tableName, int customerId) //USer tablodan customerId bilgisine göre kayıt getirme
        {
            try
            {
                connection.Open();
                SqlCommand cmd = new SqlCommand("GetRegUser", connection);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet DataListSales(int customerId) //Herhangibir sorgu ile listeleme
        {
            try
            {
                connection.Open();
                SqlCommand cmd = new SqlCommand("Select *From sales Where customerId="+customerId, connection);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                da.Fill(ds);
                cmd.Dispose();
        ...
        public DataSet DataListSales(int customerId, string query) //Herhangibir sorgu ile listeleme
        {
            try
            {
                connection.Open();
                SqlCommand cmd = new SqlCommand("Select *From sales Where customerId=" + customerId+" and bookId="+query, connection);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                da.Fill(ds);
                cmd.Dispose();
